...
For odd squarefree `n` in `2..10000`: `4055` of `9999` rows.
The identity `lambda(n)/phi(n) = lcm(p-1)/prod(p-1)` holds exactly for
`4055/4055` of them (integer arithmetic, no floats).
...
1. The published transfer replicates exactly (`r = -0.5804`).
2. The classical covariate `Z` alone is a stronger predictor of the target
   (`r = -0.8631`) than the path gap itself.
